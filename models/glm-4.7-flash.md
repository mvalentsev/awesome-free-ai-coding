...
**2 rows on the list serve `glm-4.7-flash` free:** AIHubMix (free models) and Z.ai (Zhipu GLM). None asks for a card. A live probe confirmed each one on 2026-09-24 and reads them again twice a week.
...
- Call it: `glm-4.7-flash-free` at `https://aihubmix.com/v1`, with a key in `AIHUBMIX_API_KEY` from <https://aihubmix.com/token>
...
- Call it: `glm-4.7-flash` at `https://api.z.ai/api/paas/v4`, with a key in `ZAI_GLM_API_KEY` from <https://z.ai/manage-apikey/apikey-list>
