...
**6 rows on the list serve `nemotron-3-super` free:** OpenRouter (free models), Kilo Code, Requesty, NVIDIA NIM (build.nvidia.com), AIHubMix (free models) and LLMTR. None asks for a card; Kilo Code answers with no account at all. A live probe confirmed each one on 2026-09-24 and reads them again twice a week.
...
- Call it: `nvidia/nemotron-3-super-120b-a12b:free` at `https://openrouter.ai/api/v1`, with a key in `OPENROUTER_API_KEY` from <https://openrouter.ai/settings/keys>
...
- Call it: `nvidia/nemotron-3-super-120b-a12b:free` at `https://api.kilo.ai/api/gateway`, with no key
...
- Call it: `nvidia/nemotron-3-super-120b-a12b` at `https://router.requesty.ai/v1`, with a key in `REQUESTY_API_KEY` from <https://app.requesty.ai/api-keys>
...
- Call it: `nvidia/nemotron-3-super-120b-a12b` at `https://integrate.api.nvidia.com/v1`, with a key in `NVIDIA_NIM_API_KEY` from <https://build.nvidia.com>
...
- Call it: `nemotron-3-super-120b-a12b-free` at `https://aihubmix.com/v1`, with a key in `AIHUBMIX_API_KEY` from <https://aihubmix.com/token>
...
- Call it: `nvidia/nemotron-3-super-120b-a12b` at `https://llmtr.com/v1`, with a key in `LLMTR_API_KEY` from <https://llmtr.com/dashboard/api-keys>
