...
- Note: pick models with the :free suffix: every id carrying it is priced 0/0, 20 on 2026-09-23, and the lane rotates, so a new id waits two weeks for the Models column. nemotron-3.5-content-safety, a guardrail classifier, is left out; openrouter/free is the free-models router, ling-3.0-flash-fin and ling-3.0-flash-sante are finance- and health-tuned, nemotron-3-nano-omni is a perception sub-agent, and LiquidAI warns lfm-2.5-2.6b off agentic coding. For Claude Code, OpenRouter's cookbook sets ANTHROPIC_BASE_URL to https://openrouter.ai/api with ANTHROPIC_API_KEY empty and a :free id as ANTHROPIC_MODEL
